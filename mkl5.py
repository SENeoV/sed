...
def generate_cmake_lists(root_folder):
    sources_folder_name = "main"
    sources_folder = os.path.join(root_folder, sources_folder_name)
    cmake_file_path = os.path.join(sources_folder, "CMakeLists.txt")
    ...
    for foldername, subfolders, filenames in os.walk(os.path.join(root_folder, sources_folder_name)):
        for filename in filenames:
            file_path = os.path.join(foldername, filename)
            relative_path = os.path.relpath(file_path, sources_folder)
            relative_path = relative_path.replace("\\", "/")

            # if filename.endswith(('.c', '.h')):
            if filename.endswith(('.c', '.cpp')):
                source_files.append(f'\t\t\t\t"{relative_path}"\n')
            elif filename.endswith('file.txt'):
                embed_txt_files.append(f'\t\t\t\t\t\t"{relative_path}"\n')
            elif filename.endswith(('.png', '.html', '.js')):
    ...
    for foldername, _, _ in os.walk(os.path.join(root_folder, sources_folder_name)):
        if foldername != sources_folder:
            relative_path = os.path.relpath(foldername, sources_folder)
            relative_path = relative_path.replace("\\", "/")
